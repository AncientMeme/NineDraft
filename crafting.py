...
class GridCrafterView(tk.Frame):
    ...
    def __init__(self, master, input_size):
        ...
        self._output_button = tk.Button(self, text=f"=> Craft =>")
        self._output_button.pack(side=tk.LEFT)
    ...
    def render(self, key_stack_pairs, selected):
        ...
        for key, stack in key_stack_pairs:
            ...
            if key == "output":
                ...
                self._output_grid.draw_cell((0, 0), stack, active=selected == "output")
            else:
                # Task 2.2 Crafting: Draw input cells
                self._input_grid.draw_cell(key, stack, active=selected == key)
    ...
    def bind_for_id(self, event, callback):
        ...
        self._input_grid.bind(event, lambda e: callback(self._input_grid.xy_to_grid((e.x, e.y)), e))
        self._output_button.bind(event, lambda e: callback("craft", e))
...
class GridFurnaceView(GridCrafterView):
    """A tkinter widget used to display smelting with a 1x2 grid as input and a single cell as output"""
    ...
    def __init__(self, master, input_size):
        ...
        Parameters:
            master (tk.Frame | tk.Toplevel | tk.Tk): Tkinter parent widget
        """
        super().__init__(master, (3, 1))

        # Task 2.2 Crafting: Create widgets here
        # self._top_frame = tk.Frame()
        # self._top_frame.pack(side=tk.TOP)
        #
        # self._input_grid = ItemGridView(self, (3, 1))
        # self._input_grid.pack(side=tk.LEFT)

        self._output_button.config(text="=> Smelt =>")

        self._input_grid.grid_to_xy_centre((1, 0))
    ...
    def render(self, key_stack_pairs, selected):
        ...
        for key, stack in key_stack_pairs:
            ...
            if key == (1, 0):
                key = (2, 0)
            new_pairs.append((key, stack))

        super().render(new_pairs, selected)

        # Redrawing the middle grid as flame
        x1, y1, x2, y2 = self._input_grid.grid_to_xy_box((1, 0))
        x = (x1 + x2) // 2
        y = (y1 + y2) // 2
        fire_icon = tk.PhotoImage(file="images/fire.gif").subsample(4, 4)
        self._input_grid.create_image((x, y), image=fire_icon)
        self._input_grid.image = fire_icon
    ...
    def bind_for_id(self, event, callback):
        ...
        def _bind_bottom_grid(e):
            position = self._input_grid.xy_to_grid((e.x, e.y))
        ...
        self._input_grid.bind(event, _bind_bottom_grid)
        self._output_button.bind(event, lambda e: callback("craft", e))
        self._output_grid.bind(event, lambda e: callback("output", e))
...
class CraftingWindow(tk.Toplevel):
    ...
    def _load_crafter_view(self):
        ...
        view_classes = {
            "furnace": GridFurnaceView
        }

        class_ = view_classes.get(self._title, GridCrafterView)
        self._source_views['crafter'] = crafter_view = class_(self, self._sources['crafter'].get_input_size())
